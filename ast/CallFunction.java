...
public class CallFunction extends Node {
    ...
    private void put_params_value(Func f, HashMap<String,Object> variables, List<Func> functions, HashMap<String, Data> datas, Stack<ReturnList> returns)
    {
        int i=0;
        if(f.getParams() != null)
        {
    ...
    private void put_returns_value(HashMap<String,Object> variables, List<Func> functions, HashMap<String, Data> datas, Stack<ReturnList> returns)
    {
        ...
        {
            variables.put(lv.getID().getName(), returns.peek().getReturnList().get(i).getE().interpret(variables, functions, datas, returns));
            i+=1;
        }
    }
