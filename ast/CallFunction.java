...
public class CallFunction extends Expr {
    ...
    private void put_params_value(Func f, HashMap<String,Object> variables, List<Func> functions, HashMap<String, Data> datas, Stack<ExprList> returns)
    {
        int i=0;
        if(f.getParams() != null)
        {
            ...
            {
                 f.getvFunc().put(n.getId().getName(),e.getList().get(i).interpret(variables, functions, datas, returns));
            }
        }
    }

    private void put_returns_value(HashMap<String,Object> variables, List<Func> functions, HashMap<String, Data> datas, Stack<ExprList> returns)
    {
        int i=0;
        for(LValue lv : l.getList())
        {
            variables.put(lv.getID().getName(), returns.peek().getList().get(i).interpret(variables, functions, datas, returns));
            i+=1;
        }
    }
